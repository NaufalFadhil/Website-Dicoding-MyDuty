    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        if (list_data.length > 0) {
            ...
            data_app += '<thead>' +
                ...
                '<th>Batas Akhir Pengumpulan Tugas</th>' +
            ...
            for (i in list_data) {
                ...
                data_app +=
                    ...
                    '<td>' + list_data[i].matkul + ' </td>' +
                    '<td>' + list_data[i].tanggal + ' </td>' +
                    '<td>' + list_data[i].batas + ' </td>' +
                    '<td>' + list_data[i].proses + ' </td>' +
                    '<td>' + list_data[i].tugas + ' </td>' +
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#ebatas").val(list_data[i].batas);
                $("#eproses").val(list_data[i].proses);
                $("#etugas").val(list_data[i].tugas);
    ...
    if (localStorage.list_data && localStorage.id_data) {
        ...
        for (i in list_data) {
            if (list_data[i].id_data == id) {
                ...
                $("#lmatkul").val(list_data[i].matkul);
                $("#ltanggal").val(list_data[i].tanggal);
                $("#lbatas").val(list_data[i].batas);
                $("#lproses").val(list_data[i].proses);
                $("#ltugas").val(list_data[i].tugas);
                list_data.splice(idx_data, 1);
            }
            idx_data++;
        }
